 ...
 * 4. Implement all the TODOs described in comments below and save your work.
 *
 *
...
const Product = class Product {
    ...
    constructor(productStr) {
        const [name, price, availability] = productStr.split(',');
        this.name = name;
     ...
     * Take note, for 'Availability' we are not returning 'In Stock' or 'Out of Stock' but 'Yes' or 'No'
     * Reference: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array
    ...
    static printProducts = (products) => {
        products.forEach(product => {
            const yesOrNo = product.availability === 'In Stock' ? 'Yes' : 'No';
            const priceStr = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(product.price);
            console.log(`Product: ${product.name}, Cost: ${priceStr}, Availability: ${yesOrNo}`);
        });
    }
